...
<cfprocessingdirective pageencoding="utf-8" />
	...
	<cffunction name="call" returntype="struct" access="remote" returnformat="JSON" hint="Acts as a interface for frontend Javascript via ajax to call backend CFC methods, passing along argument data as well." >
		<cfargument name="Component" type="string" required="true" hint="The name of the CFC method you want to call." />
		...
		<cfif validateAuthKey(Key=arguments.AuthKey) IS false >

			<cfheader statuscode="500" />
			<cfset stReturnData.status = "NOK" />
			<cfset stReturnData.message = "Action not allowed" />
			<cfreturn stReturnData />

		</cfif>

		<cfreturn invoke("CHANGE THIS!!!", arguments.Function, arguments.Parameters) />
